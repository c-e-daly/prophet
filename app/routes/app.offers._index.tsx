...
import { getShopOffers, type GetShopOffersResult } from "../lib/queries/supabase/getShopOffers";
import { getAuthContext } from "../lib/auth/getAuthContext.server";
import { useAppNavigate, useAppUrl } from "../utils/navigation";
import type { OfferRow, OfferStatusType } from "../lib/types/dbTables";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const statuses = statusParam
    ? statusParam.split(",").map((s) => s.trim()).filter(Boolean) as OfferStatusType[]
    : [OfferStatusEnum.AutoAccepted, OfferStatusEnum.PendingReview] as OfferStatusType[];
  
  const host = url.searchParams.get("host");

  // Fetch offers using RPC - NO REQUEST PARAMETER
  ...
  // Create status options from enum - STANDARDIZED APPROACH
