...
import { getCounterOfferEditorData, upsertCounterOffer } from "../lib/queries/supabase/getShopCounterOfferData";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  // For new counters, offersID is required
  if (isNew && !offersID) {
...
const editorData = await getCounterOfferEditorData(
  ...
  offersID ? Number(offersID) : undefined
);
  
return json({
  isNew,
  counterID: isNew ? null : Number(id),
  offersID: offersID ? Number(offersID) : editorData.offers?.id,
  ...editorData,
  authContext: {
    shops,
    user, 
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  // Redirect back to the offer detail page
...
export default function CounterOfferEditor() {
  ...
  const [counterType, setCounterType] = useState<CounterType>(
    counterOffers?.counterType || "percent_off_order"
  );
  const [counterConfig, setCounterConfig] = useState<CounterConfig>(
    counterOffers?.counterConfig || getDefaultConfig("percent_off_order")
  );
  ...
  return (
    <Page
      ...
      backAction={{ 
        content: "Back to Offer", 
        url: `/app/offers/${offersID}` 
      }}
    >
      <Layout>
        <Layout.Section>
