...
import { Page, Card, Button, Text, IndexTable, InlineStack, BlockStack, TextField,
  Select, Badge, Box,} from "@shopify/polaris";
import React, { useMemo, useState, useRef } from "react";
import { useIndexResourceState } from "@shopify/polaris";
import { formatCurrencyUSD, formatDateTime } from "../utils/format";
import { getShopProductVariants, type VariantRow,
} from "../lib/queries/supabase/getShopProductVariants";
...
export default function PriceBuilderIndex() {
  ...
  // Hidden input ref so we can write JSON and submit without setState race
  const hiddenInputRef = useRef<HTMLInputElement>(null);

  const onBulkEdit = () => {
    const variantIds = selectedResources.map((id) => Number(id));
    if (hiddenInputRef.current) {
      hiddenInputRef.current.value = JSON.stringify(variantIds);
    }
      ...
      {selectedResources.length > 0 && (
        <Box paddingBlockEnd="200">
          <InlineStack align="end">
            <Button variant="primary" onClick={onBulkEdit}>
              Bulk Edit: {allResourcesSelected ? "All" : selectedResources.length.toString()} items
            </Button>
          </InlineStack>

          {/* Posts IDs to /app/pricebuilder/move, which flashes + redirects */}
          <Form id="pb-flash-move" method="post" action="/app/pricebuilder/move">
            <input ref={hiddenInputRef} type="hidden" name="variantIds" defaultValue="[]" />
          </Form>
        </Box>
      )}
