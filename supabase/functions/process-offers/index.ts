...
// Keep GraphQL here (per your vote)
...
type OfferPayload = {
  ...
  items?: Array<{
    ...
    productURL?: string;
    variantID?: number;
    variantSKU?: string;     // NOTE: prefer variantSKU in DB layer; tolerate sku in payload if needed upstream
...
serve(async (req) => {
  ...
  // 1) Upsert CONSUMER (+ link to SHOP via consumerShop). Returns consumersID + shopsID.
  ...
  // 2) Upsert CART (linked to consumer + shop). Returns cartsID.
  ...
  // 3) Upsert CART ITEMS (linked to consumer + shop + cart).
  ...
  // 4) Upsert OFFER (linked to consumer + shop + cart).
  ...
  // 5) Read the offer row (assumes triggers/functions evaluate status, create discounts on AutoAccept, etc.)
  ...
  // If not AutoAccept, short-circuit with what we have
  ...
  // 6) Locate the discount row created by your AFTER UPDATE trigger (AutoAccept → discounts row)
  let discountsID: number | null = offerRow.discounts ?? null;
  if (!discountsID) {
  ...
  if (!discountsID) {
    return json({
      ...
      error:
        "Offer is AutoAccept but no discount row exists. Verify the AFTER UPDATE trigger that calls create_discount_for_autoaccepted_offer.",
    }, { status: 500 });
  }

  // 7) Build Shopify variables (RPC renamed per your instruction)
  const { data: variables, error: varsErr } = await supabase.rpc(
    "process_offer_shopify_discount",
    { discountsId: discountsID }, // RPC arg name uses discountsId per your SQL; adjust if renamed to discountsID
  );
  if (varsErr) {
    return json({ step: "build-variables", error: varsErr.message, discountsID }, { status: 500 });
  }

  // 8) Fetch shop domain + access token
