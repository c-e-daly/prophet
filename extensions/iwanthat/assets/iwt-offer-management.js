// iwt-offer-management.js - control offer workflows
...
async function waitForDependencies(maxWait = 5000) {
    const requiredFunctions = ['iwtOpenOfferModal', 'iwtCloseModal', 'iwtHandleSubmit'];
...
// Check if URL contains auto-open parameters
...
function setupEventListeners() {
    ...
    // Submit button event listener - IMPROVED ERROR HANDLING
    ...
    if (submitBtn) {
        submitBtn.addEventListener('click', async (event) => {
            ...
            if (typeof window.iwtHandleSubmit === 'function') {
                ...
                alert("Submit function is not ready. Please refresh and try again.");
            }
        });
        console.log("✅ Submit button listener attached");
    }

    // Setup dynamic event listeners using event delegation
...
// Setup event listeners for dynamically created elements using delegation
...
window.iwtDebugManagement = function() {
    ...
    console.log("Form element:", !!document.getElementById('iwt-form'));
    ...
    const form = document.getElementById('iwt-form');
    if (form) {
        const inputs = form.querySelectorAll('input');
        console.log("Form inputs:", Array.from(inputs).map(input => ({
            id: input.id,
            name: input.name,
            type: input.type
        })));
